...
3. Open the downloaded `.msi` file and click through the installer — just keep clicking **Next** and **Install**
4. When it asks about "Tools for Native Modules" — you can leave that unchecked
5. Click **Finish** when done

✅ **Done when:** The installer shows "Finished" or "Complete."

---

## Step 2 — Install Claude Code

1. Go to **https://claude.ai/download** on your PC
2. Download the Windows installer (`.exe` file)
3. Open the installer and click through it
4. Open Claude from the Start menu and sign in
5. You need a **Claude Pro plan** ($20/mo) — if you don't have one, click your name → Upgrade to Pro
...
### Open PowerShell:
1. Click the **Start button** (Windows key) at the bottom left
2. Type `PowerShell`
3. Click **Windows PowerShell** — a blue window opens with a cursor

> **Don't use Command Prompt** (cmd) — it won't work. Use PowerShell specifically.
...
> **To paste in PowerShell:** Right-click anywhere in the window (that pastes — there's no Ctrl+V in the old PowerShell)
> If you have **Windows Terminal**, you can use Ctrl+V normally.
...
| GHL token | Paste your `pit-...` token from Step 3 |
| GHL Location ID | Paste your Location ID from Step 4 |
| 21st.dev API key | Paste your key from Step 5 (or press Enter to skip) |
...
1. Right-click Claude in the taskbar → **Close window** (or press **Alt + F4**)
2. Open Claude again from the Start menu
...
Paste this in PowerShell to run a quick check:
```
...
See **[QUICK_REFERENCE.md](QUICK_REFERENCE.md)** for a one-page list of copy-paste prompts for every daily situation.

---

## Something didn't work?

See **[TROUBLESHOOTING_WINDOWS.md](TROUBLESHOOTING_WINDOWS.md)** for step-by-step fixes.

Quick answers:

**"irm is not recognized" or "iex is not recognized"**
→ You're using Command Prompt, not PowerShell. Close it, open PowerShell from the Start menu instead.

**"Running scripts is disabled on this system"**
→ Run this first: `Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope CurrentUser` then try again

**"Node.js not installed" error**
→ Go to nodejs.org, download the LTS version, install it, then **close and reopen PowerShell** and run the installer again

**GHL tools not working**
→ Close Claude completely (right-click taskbar → Close) and reopen it — MCPs load fresh on startup
